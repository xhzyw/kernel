...
struct nxp_adc_info *gec_light = NULL;
struct iio_dev *gec_light_iio = NULL;
...
#define ADC_CHANNEL_MAP(_id) {	\
    .consumer_dev_name =  DEV_NAME_LIGHT,	\
}
...
static int nxp_adc_setup(struct nxp_adc_info *adc, struct platform_device *pdev)
{
	...
	if ((irq >= 0) || (NR_IRQS > irq)) {
		ret = request_irq(irq, nxp_adc_isr, 0, DEV_NAME_LIGHT, adc);
		if (0 == ret)
			interrupt = 1;
		ret = 0;
	}
...
//没有定义
		...
		*val = value;
		gec_light->value = value;
		printk("<0>""gec_light_value --nxp_read_raw---  = %d\n", gec_light->value);
		if (0 >= wait) {
			ADC_UNLOCK(&adc->lock, flags);
			return -EINVAL;
		}
...
//zr	
		//gec_light = = container_of(work, struct nxp_adc_info, monitoring_work.work);
//end
	struct iio_chan_spec const *chan;
	int val = 0;
	int val2 = 0;
	...
	//gec_light->value = (18*val*1000)/4096;
...
static int gec_light_open(struct inode *inode, struct file *file)
{
	return 0;
}

static ssize_t gec_light_read(struct file *file, char __user *buffer, size_t size, loff_t *pos)
{
	struct iio_chan_spec const *chan;
	...
	chan = &nxp_adc_iio_channels[1];
	nxp_read_raw(gec_light_iio, chan, &val, &val2, 0);
	
	printk("<0>""gec_light_value = %d\n", gec_light->value);
	adc_value = gec_light->value;
...
///zoroe
	ret = nxp_adc_setup(gec_light, pdev);
	if (0 > ret) {
		pr_err("Fail: setup iio ADC device\n");
		goto err_iio_free;
	}
//end	
	iio->name = DEV_NAME_LIGHT;
...
//zoroe
	gec_light->map = nxp_adc_iio_maps;
//end
	gec_light_iio = iio;
#if defined(CONFIG_ARM_NXP_CPUFREQ_BY_RESOURCE)
	adc->isCheckedCount = 0;
