...
static unsigned int gec6818_channels;
...
struct nxp_adc_info *gec_adc = NULL;
struct iio_dev *gec_adc_iio = NULL;
...
#ifdef ADC_USING_PROTOTYPE //没有定义
#else
...
//没有定义
		...
		*val = value;
		gec_adc->value = value;
		printk("<0>""gec_adc_value --nxp_read_raw---  = %d\n", gec_adc->value);
		if (0 >= wait) {
			ADC_UNLOCK(&adc->lock, flags);
			return -EINVAL;
		}
...
//zr	
		//gec_adc = = container_of(work, struct nxp_adc_info, monitoring_work.work);
//end
	struct iio_chan_spec const *chan;
	int val = 0;
	int val2 = 0;
	...
	int interval;
	int num_grade;
printk("<0>""__________________gec6818_adc_workfunc___________\n");
	...
	//gec_adc->value = (18*val*1000)/4096;
...
//............................................................................
static int gec_adc_open(struct inode *inode, struct file *file)
{
	return 0;
}

static ssize_t gec_adc_read(struct file *file, char __user *buffer, size_t size, loff_t *pos)
{
	struct iio_chan_spec const *chan;
	...
	int val2 = 0;
	int adc_value = 0;
	printk("<0>""------------gec6818_channels = %d\n", gec6818_channels);
	chan = &nxp_adc_iio_channels[gec6818_channels];
	nxp_read_raw(gec_adc_iio, chan, &val, &val2, 0);
	
	printk("<0>""gec_adc_value = %d\n", gec_adc->value);
	adc_value = gec_adc->value;
...
static long gec6818_adc_ioctl(struct inode *inode, struct file *file, unsigned int cmd, unsigned long arg)
{
	//gec6818_channels = cmd;
	//printk("<0>""------------ioctl__gec6818_channels = %d\n", gec6818_channels);
	switch(cmd)
	{
		case Channels1:
		{
			gec6818_channels = arg;	
			break;		
		}
		case Channels2:
		{
...
///zoroe
	ret = nxp_adc_setup(gec_adc, pdev);
	if (0 > ret) {
		pr_err("Fail: setup iio ADC device\n");
		goto err_iio_free;
	}
...
//zoroe
	...
	if(ret < 0)
	{
		printk("<0>""can not register adc_misc\n");
	}
//end
...
//nxp_adc_device的路径为arch/arm/mach-s5p6818/devices.c中
static struct platform_driver nxp_adc_driver = {
	.probe		= nxp_adc_probe,     //文件操作
	.remove		= __devexit_p(nxp_adc_remove),//文件移除
	.suspend	= nxp_adc_suspend,//中止
	.resume		= nxp_adc_resume,//恢复
	.driver		= {
		.name	= DEV_NAME_ADC,//为nxp-adc
		.owner	= THIS_MODULE,
	},
};
