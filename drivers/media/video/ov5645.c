...
static struct i2c_client *ov5645_client;
...
static int ov5645_init(struct v4l2_subdev *sd)
{
	...
	msleep(10);
	reg_write_array(client, OV5645_focus_constant);
	//reg_write_array(client, OV5645_focus_constant1);
#endif

#if 0
	reg_write_array(client, OV5645_auto_focus_init_0723);
	//reg_write_array(client, OV5645_auto_focus_init);
	
	//OV5645_FOCUS_OVT_AFC_Init(sd);
	msleep(50);
	reg_write_array(client, OV5645_reg_init_2lane);
	msleep(10);
#endif
...
//    reg_write_array(client, OV5645_reg_stop_stream);
    ...
    if (enable) {
        if (!priv->win || !priv->cfmt) {
         ...
         /* write init regs */
        if (!priv->initialized) {
  //       ret = reg_write_array(client, ov5645_svga_init_regs);
            ret = ov5645_init(sd);
            if (ret < 0) {
...
//mdelay(100);
//printk("hdc %s auto constant\n", __func__);
//reg_write_array(client, OV5645_focus_constant);	//not work;
//mdelay(50);
    } else {
        reg_write_array(client, OV5645_reg_stop_stream);
	printk("%s: OV5645_reg_stop_stream\n", __func__);
...
{
    struct ov5645_priv *priv, *dummy_priv;
    struct v4l2_subdev *sd, *dummy_sd;
    int ret;
#if 1
...
#if 0
/*****************ace 20150722 test ********************/
	dummy_priv = kzalloc(sizeof(struct ov5645_priv), GFP_KERNEL);  
    if (!dummy_priv)
        return -ENOMEM;
    printk("%s...................................\n",__func__);

	ov5645_client = client;
	ov5645_client->addr = 0x3c;    
    ov5645_priv_init(dummy_priv);

	dummy_sd = &dummy_priv->subdev;  


	 ov5645_video_probe(ov5645_client);

    v4l2_i2c_subdev_init(dummy_sd, ov5645_client, &ov5645_subdev_ops);
	    printk("%s.................1..................\n",__func__);

   strcpy(dummy_sd->name, "OV5645 0-0038");
    dummy_sd->flags |= V4L2_SUBDEV_FL_HAS_DEVNODE;
    dummy_priv->pad.flags = MEDIA_PAD_FL_SOURCE;
    dummy_sd->entity.type = MEDIA_ENT_T_V4L2_SUBDEV_SENSOR;
    dummy_sd->entity.ops  = &ov5645_media_ops;
    if (media_entity_init(&dummy_sd->entity, 1, &dummy_priv->pad, 0)) {
        dev_err(&ov5645_client->dev, "%s: failed to media_entity_init()\n", __func__);
	 kfree(dummy_priv);    //ace 20150721 
        return -ENOENT;
    }

    ret = ov5645_initialize_ctrls(dummy_priv);
    if (ret < 0) {
        printk(KERN_ERR "%s: failed to initialize controls\n", __func__);
	 kfree(dummy_priv);    //ace 20150721 
        return ret;
...
//	nxp_v4l2_capture_set_sensor_subdev(dummy_sd);    //ace 20150721 
/*****************ace 20150722 test ********************/
#endif
